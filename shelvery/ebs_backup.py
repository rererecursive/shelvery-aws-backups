...
class ShelveryEBSBackup(ShelveryEC2Backup):
    ...
    def get_existing_backups(self, tag_prefix: str, volume_ids: list) -> List[BackupResource]:
    ...
    def get_entities_to_backup(self, tag_name: str, volume_ids: list) -> List[EntityResource]:
        volumes = self.collect_volumes(tag_name, volume_ids)
        return list(
            map(
    ...
    def collect_volumes(self, tag_name: str, volume_ids: list):
        load_volumes = True
        next_token = ''
        all_volumes = []
        while load_volumes:
            tagged_volumes = self.ec2client.describe_volumes(
                ...
                VolumeIds=volume_ids    # An empty value will return all volumes
            )
